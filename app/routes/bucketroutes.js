...
app.post("/", (req, res) => {
  ...
  if (!req.headers.authorization) {
    ...
    let query = `INSERT INTO bucket SET bucket_name = "${values.bucket_name}"`;
  ...
  if (!req.headers.authorization) {
    ...
    conn.query(query, (err, response) => {
      if (err) {
        ...
        res.status(200).send({ status: 0, message: "Updated successfully" });
      }
    });
  }
});
  ...
  if (!req.headers.authorization) {
    ...
    conn.query(query, (err, response) => {
      if (err) {
        ...
        res.status(200).send({ status: 0, message: "deleted Successfully" });
      }
    });
  }
});
